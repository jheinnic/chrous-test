...
export class PageContainerComponent implements OnInit, OnDestroy
{
  ...
  )
  {
    ...
    this.rowCount = this.gridHeight / this.cellHeight;
    this.colCount = this.gridWidth / this.cellWidth;
    ...
    for (let ii = 0; ii < cellCount; ii++) {
      ...
      this.cellStyles[ii] = {
        'background-color': colorOne.isDark() ? colorOne.toString() : colorTwo.toString(),
        color: colorOne.isDark() ? colorTwo.toString() : colorOne.toString()
      };
    }
  }
